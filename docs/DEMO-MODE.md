...
Guest state is kept in browser `localStorage` under ReForge-specific keys, including a separate Community Circle namespace for private demo notes. The guest workspace does not call protected tRPC procedures and does not write anonymous journal, check-in, assessment, goal, rule, or community data to the shared database. A browser profile therefore sees only its own demo state for that site origin.
...
3. The interface must state that the experience is a demo, that local data can be reset, and that Community Circle notes are not public posts.
