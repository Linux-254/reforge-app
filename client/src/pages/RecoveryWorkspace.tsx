...
import { phaseForScore, recoveryPhases } from "@/lib/progressPhases";
...
export function ProgressPage() {
  ...
  const { user } = useAuth();
  const query = trpc.dashboard.getDimensionScores.useQuery(undefined, { enabled: !demoMode && Boolean(user) });
  const [selectedDimensionId, setSelectedDimensionId] = useState<number | null>(null);
  const historyInput = useMemo(() => ({ dimensionId: selectedDimensionId ?? 0, limit: 30 }), [selectedDimensionId]);
  const historyQuery = trpc.dashboard.getDimensionHistory.useQuery(historyInput, { enabled: !demoMode && Boolean(user) && selectedDimensionId !== null, retry: false });
  const dimensions = query.data ?? [];
  const averageScore = dimensions.length ? Math.round(dimensions.reduce((total, dimension) => total + dimension.score, 0) / dimensions.length) : 0;
  const activePhaseIndex = phaseForScore(averageScore);
  const selectedDimension = dimensions.find((dimension) => dimension.dimensionId === selectedDimensionId);

  return <Workspace><div className="mx-auto max-w-6xl space-y-7">
    <PageHeading eyebrow="Your whole-life view" title="Progress across 21 dimensions" description="Small, honest steps count. This view helps you notice where care is already taking root." />
    {query.isLoading ? <div className="space-y-4"><Skeleton className="h-32 w-full rounded-3xl" /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{Array.from({ length: 6 }).map((_, index) => <Skeleton key={index} className="h-28 rounded-2xl" />)}</div></div> : query.isError ? <Card className="rounded-2xl border-destructive/30"><CardContent className="pt-6"><p className="text-sm text-destructive">We could not load your progress right now. Please try again in a moment.</p><Button variant="outline" className="mt-4 rounded-full" onClick={() => void query.refetch()}>Try again</Button></CardContent></Card> : <>
      <Card className="overflow-hidden rounded-[2rem] border-border/70 bg-card/80 shadow-none"><CardHeader><div className="flex flex-wrap items-end justify-between gap-4"><div><CardTitle className="font-serif text-2xl">The season you are in</CardTitle><CardDescription>These phases are a reflection aid, not a clinical measure or a race.</CardDescription></div><Badge className="rounded-full bg-primary/10 text-primary hover:bg-primary/10">{averageScore}% whole-life signal</Badge></div></CardHeader><CardContent><div className="mb-5 h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-[width] duration-500" style={{ width: `${Math.max(3, averageScore)}%` }} /></div><div className="grid gap-2 sm:grid-cols-4">{recoveryPhases.map((phase, index) => <div key={phase.label} className={`rounded-2xl border p-4 ${index === activePhaseIndex ? "border-primary/40 bg-primary/10" : "border-border/50 bg-muted/20"}`}><div className="flex items-center justify-between gap-2"><p className="font-serif text-lg">{phase.label}</p>{index === activePhaseIndex && <Check className="h-4 w-4 text-primary" aria-label="Current phase" />}</div><p className="mt-1 text-xs font-medium text-muted-foreground">{phase.range}%</p><p className="mt-2 text-xs leading-5 text-muted-foreground">{phase.description}</p></div>)}</div></CardContent></Card>
      <div className="grid gap-6 lg:grid-cols-[1fr_320px] lg:items-start"><Card className="rounded-2xl border-border/70 bg-card/80 shadow-none"><CardHeader><CardTitle className="font-serif text-2xl">Choose a dimension</CardTitle><CardDescription>Select any card to see its recent score history and a plain-language way to read the signal.</CardDescription></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{dimensions.map((dimension) => { const phase = recoveryPhases[phaseForScore(dimension.score)]; return <button key={dimension.dimensionId} type="button" aria-pressed={selectedDimensionId === dimension.dimensionId} onClick={() => setSelectedDimensionId(dimension.dimensionId)} className={`rounded-2xl border p-4 text-left transition-colors hover:border-primary/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${selectedDimensionId === dimension.dimensionId ? "border-primary/50 bg-primary/10" : "border-border/60 bg-muted/20"}`}><div className="flex items-start justify-between gap-3"><span className="font-serif text-lg text-foreground">{dimension.dimensionLabel}</span><span className="text-sm font-semibold text-primary">{dimension.score}%</span></div><Progress value={dimension.score} className="mt-4 h-2" /><div className="mt-3 flex items-center justify-between gap-2 text-xs text-muted-foreground"><span>{phase.label}</span><span>{dimension.capturedOn ? new Date(dimension.capturedOn).toLocaleDateString() : "No check-in yet"}</span></div></button>; })}</div></CardContent></Card>
        <Card className="rounded-2xl border-border/70 bg-card/85 shadow-none lg:sticky lg:top-6"><CardHeader><CardTitle className="font-serif text-2xl">{selectedDimension?.dimensionLabel ?? "A closer look"}</CardTitle><CardDescription>{selectedDimension ? "A private view of the signals you have recorded for this dimension." : "Choose a dimension to open its detail view."}</CardDescription></CardHeader><CardContent className="space-y-5">{selectedDimension ? <><div className="flex items-end justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Current signal</p><p className="mt-1 font-serif text-5xl text-primary">{selectedDimension.score}%</p></div><Badge variant="outline" className="rounded-full">{recoveryPhases[phaseForScore(selectedDimension.score)].label}</Badge></div><p className="text-sm leading-6 text-muted-foreground">{recoveryPhases[phaseForScore(selectedDimension.score)].description}</p><div><div className="mb-3 flex items-center justify-between"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Recent history</p><TrendingUp className="h-4 w-4 text-primary" aria-hidden="true" /></div>{historyQuery.isLoading ? <Skeleton className="h-28 rounded-xl" /> : historyQuery.isError ? <p className="text-sm text-destructive">History is unavailable right now.</p> : historyQuery.data?.length ? <div className="space-y-3">{historyQuery.data.slice(0, 6).map((point) => <div key={`${point.capturedOn}-${point.score}`}><div className="mb-1 flex justify-between text-xs"><span className="text-muted-foreground">{new Date(point.capturedOn).toLocaleDateString()}</span><span className="font-semibold text-primary">{point.score}%</span></div><Progress value={point.score} className="h-2" /></div>)}</div> : <p className="text-sm leading-6 text-muted-foreground">No score history yet. A check-in will give this dimension its first signal.</p>}</div></> : <div className="rounded-2xl border border-dashed border-border/70 bg-muted/20 p-6 text-center"><Target className="mx-auto h-7 w-7 text-primary" aria-hidden="true" /><p className="mt-3 font-serif text-xl">Progress is a practice</p><p className="mt-2 text-sm leading-6 text-muted-foreground">Select a card to understand the signal without turning it into a verdict.</p></div>}</CardContent></Card></div>
    </>}</div></Workspace>;
}
...
export function JournalPage() {
  ...
  const [entryDimension, setEntryDimension] = useState("all");
  ...
  const dimensionsQuery = trpc.onboarding.getDimensions.useQuery(undefined, { staleTime: 5 * 60 * 1000, enabled: !demoMode && Boolean(user) });
  ...
  const listQuery = trpc.journal.list.useQuery(listInput, { enabled: !demoMode && Boolean(user) });
  const utils = trpc.useUtils();
  const createMutation = trpc.journal.create.useMutation({
    onSuccess: async () => {
      setBody("");
      setActivePrompt(undefined);
      setEntryDimension("all");
  ...
  const dimensionLabelById = new Map((dimensionsQuery.data ?? []).map((dimension) => [dimension.id, dimension.label]));
  const selectedEntryDimensionId = entryDimension === "all" ? undefined : Number(entryDimension);
      ...
      <div className="grid gap-6 lg:grid-cols-[.88fr_1.12fr] lg:items-start">
        <Card className="rounded-2xl border-border/70 bg-card/85 shadow-none">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2"><Label htmlFor="journal-entry-dimension">Life dimension <span className="font-normal text-muted-foreground">(optional)</span></Label><select id="journal-entry-dimension" value={entryDimension} onChange={(event) => setEntryDimension(event.target.value)} className="h-10 w-full rounded-xl border bg-background px-3 text-sm" disabled={dimensionsQuery.isLoading}><option value="all">Whole-life reflection</option>{(dimensionsQuery.data ?? []).map((dimension) => <option key={dimension.id} value={dimension.id}>{dimension.label}</option>)}</select></div>
            ...
            <Button disabled={!body.trim() || createMutation.isPending} onClick={() => createMutation.mutate({ body: body.trim(), dimensionId: selectedEntryDimensionId })} className="w-full gap-2 rounded-full"><Save className="h-4 w-4" aria-hidden="true" />{createMutation.isPending ? "Saving your page…" : "Save reflection"}</Button>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl border-border/70 bg-card/80 shadow-none">
          ...
          <CardContent className="space-y-3">
            {listQuery.isLoading ? <div className="space-y-3">{[1, 2, 3].map((item) => <Skeleton key={item} className="h-24 rounded-2xl" />)}</div> : listQuery.isError ? <div className="space-y-3"><p className="text-sm text-destructive">We could not load your private reflections right now.</p><Button variant="outline" size="sm" onClick={() => void listQuery.refetch()} className="rounded-full">Try again</Button></div> : visibleEntries.length ? visibleEntries.map((entry) => <article key={entry.id} className="rounded-2xl border border-border/50 bg-muted/30 p-4">
              <div className="mb-3 flex items-center gap-2"><Badge variant="outline" className="rounded-full text-xs">{entry.dimensionId ? dimensionLabelById.get(entry.dimensionId) ?? "Life dimension" : "Whole-life reflection"}</Badge></div>
...
function GoalCard({ goal, dimensionLabel, onChanged }: { goal: GoalRecord; dimensionLabel?: string; onChanged: () => void }) {
  ...
  const stepsQuery = trpc.goals.steps.useQuery({ goalId: goal.id }, { enabled: !demoMode && Boolean(user) });
  const historyQuery = trpc.goals.history.useQuery({ goalId: goal.id }, { enabled: !demoMode && Boolean(user) });
    ...
    <CardContent className="space-y-5">
      ...
      <div className="space-y-2">
        {stepsQuery.isLoading ? (
          <div className="space-y-2">
            {[1, 2].map((i) => (
              <Skeleton key={i} className="h-11 w-full rounded-xl bg-muted/60" />
            ))}
          </div>
        ) : stepsQuery.isError ? (
          <p className="text-sm text-destructive">Steps are unavailable right now.</p>
        ) : (
          steps.map((step) => (
            <label key={step.id} className="flex items-center gap-3 rounded-xl border border-border/60 bg-muted/20 p-3 text-sm">
              <input type="checkbox" checked={Boolean(step.doneAt)} onChange={() => toggleStepMutation.mutate({ goalId: goal.id, stepId: step.id })} className="h-4 w-4 accent-primary" />
              <span className={step.doneAt ? "text-muted-foreground line-through" : ""}>{step.title}</span>
            </label>
          ))
        )}
      </div>
      ...
      <div className="space-y-3 border-t border-border/60 pt-4">
        ...
        {historyQuery.isLoading ? (
          <div className="space-y-2 pt-1">
            ...
            <Skeleton className="h-6 w-full rounded-lg bg-muted/50" />
          </div>
        ) : historyQuery.isError ? (
          <p className="text-xs text-destructive">Status history is unavailable.</p>
        ) : historyQuery.data?.length ? (
          <div className="space-y-2">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Recent history</p>
            {historyQuery.data.slice(0, 4).map((entry) => (
              <div key={entry.id} className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
                <span className="capitalize">{entry.status}</span>
                <time dateTime={new Date(entry.changedAt).toISOString()}>{new Date(entry.changedAt).toLocaleDateString()}</time>
              </div>
            ))}
          </div>
        ) : null}
...
export function MusicPage() {
  ...
  if (profileQuery.isLoading) return <Workspace><div className="mx-auto max-w-3xl space-y-6"><Skeleton className="h-48 rounded-[2rem]" /><Skeleton className="h-72 rounded-2xl" /></div></Workspace>;
...
export function GuidesPage() {
  const demoMode = import.meta.env.VITE_DEMO_MODE !== "false";
  const { user } = useAuth();
  const dimensionsQuery = trpc.onboarding.getDimensions.useQuery(undefined, { enabled: !demoMode && Boolean(user) });
  const [dimensionId, setDimensionId] = useState<number>();
  const resourcesQuery = trpc.resources.getByDimension.useQuery({ dimensionId: dimensionId ?? 0 }, { enabled: !demoMode && Boolean(user) && Boolean(dimensionId) });
  const selectedDimension = dimensionsQuery.data?.find((dimension) => dimension.id === dimensionId);
  return <Workspace><div className="mx-auto max-w-5xl space-y-7"><section className="relative overflow-hidden rounded-[2rem] border border-border/60 bg-[#304333] text-[#f7eddc]"><img src={REFORGE_ASSETS.guides} alt="" className="absolute inset-0 h-full w-full object-cover opacity-40" /><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(39,58,43,.94),rgba(39,58,43,.45),rgba(39,58,43,.08))]" /><div className="relative max-w-2xl space-y-3 p-7 sm:p-10"><p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-amber-200"><BookOpen className="h-4 w-4" /> Context when you need it</p><h1 className="font-serif text-4xl leading-tight sm:text-5xl">Guides for the next right step.</h1><p className="text-sm leading-7 text-white/75 sm:text-base">Choose a life dimension and find activities, situation guides, and repair practices that meet you there.</p></div></section><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{dimensionsQuery.isLoading ? Array.from({ length: 9 }).map((_, index) => <Skeleton key={index} className="h-14 rounded-xl" />) : (dimensionsQuery.data ?? []).map((dimension) => <Button key={dimension.id} variant={dimensionId === dimension.id ? "default" : "outline"} className="h-auto min-h-14 justify-start whitespace-normal rounded-xl p-4 text-left" onClick={() => setDimensionId(dimension.id)}>{dimension.label}</Button>)}</div>{selectedDimension && <div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr] lg:items-start"><Card className="rounded-2xl border-primary/20 bg-primary/7 shadow-none"><CardContent className="space-y-4 p-6"><div className="flex h-11 w-11 items-center justify-center rounded-full bg-primary/12 text-primary"><Sparkles className="h-5 w-5" /></div><Badge className="rounded-full bg-primary/10 text-primary hover:bg-primary/10">{selectedDimension.label}</Badge><p className="font-serif text-2xl leading-tight">{motivationFor(selectedDimension.label)}</p><p className="text-sm leading-6 text-muted-foreground">Let this be the reason you keep the practice small enough to return to.</p></CardContent></Card>        <Card className="rounded-2xl border-border/70 bg-card/80 shadow-none">
          <CardHeader>
            <CardTitle className="font-serif text-2xl">Resources for this dimension</CardTitle>
            <CardDescription>Read, reflect, then choose one action that feels possible.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {resourcesQuery.isLoading ? (
              <div className="space-y-3">
                {[1, 2, 3].map((item) => (
                  <div key={item} className="space-y-2.5 rounded-2xl border border-border/60 bg-muted/20 p-4">
              ...
              <p className="text-sm text-destructive">We could not load these guides right now.</p>
            ) : resourcesQuery.data?.length ? (
              resourcesQuery.data.map((resource) => (
                <article key={resource.id} className="rounded-2xl border border-border/60 bg-muted/25 p-4">
                  <h3 className="font-serif text-xl">{resource.title}</h3>
                  <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{resource.body}</p>
                </article>
              ))
            ) : (
              <EmptyState title="This shelf is still being written." description="Choose another dimension or return later as new guides are added." icon={BookOpen} />
            )}
          </CardContent>
        </Card></div>}{!selectedDimension && <EmptyState title="Choose a dimension to begin." description="Every guide is connected to one of the 21 places where life can be restored." icon={BookOpen} />}</div></Workspace>;
}

export function SettingsPage() {
  const demoMode = import.meta.env.VITE_DEMO_MODE !== "false";
  const { user } = useAuth();
  const profileQuery = trpc.profile.get.useQuery(undefined, { enabled: !demoMode && Boolean(user) });
  const preferencesQuery = trpc.preferences.get.useQuery(undefined, { enabled: !demoMode && Boolean(user) });
