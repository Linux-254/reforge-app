...
export function ProgressPage() {
  ...
  return <Workspace><div className="mx-auto max-w-6xl space-y-7">
    ...
    {query.isLoading ? <div className="space-y-4"><Progress value={35} className="h-2" /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{Array.from({ length: 6 }).map((_, index) => <Card key={index} className="h-28 animate-pulse rounded-2xl bg-muted/50" />)}</div></div> : query.isError ? <Card className="rounded-2xl border-destructive/30"><CardContent className="pt-6"><p className="text-sm text-destructive">We could not load your progress right now. Please try again in a moment.</p><Button variant="outline" className="mt-4 rounded-full" onClick={() => void query.refetch()}>Try again</Button></CardContent></Card> : <>
