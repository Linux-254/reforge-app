...
export default function Dashboard() {
  ...
  const dashboardQuery = trpc.dashboard.getOverview.useQuery(undefined, { enabled: Boolean(user) });
  const moodHistoryQuery = trpc.checkIn.history.useQuery({ limit: 7 }, { enabled: Boolean(user) });
  const onboardingStatusQuery = trpc.onboarding.status.useQuery(undefined, { retry: false, enabled: Boolean(user) });
  ...
  return (
    <DashboardLayout>
      <DashboardStaggerContainer className="mx-auto max-w-6xl space-y-7">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {quickLinks.map(link => (
              <DashboardAnimatedCard key={link.path}>
                <button onClick={() => setLocation(link.path)} className="group w-full h-full rounded-2xl border border-border/65 bg-card/80 p-4 text-left transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-[0_15px_35px_-25px_rgba(39,58,43,.55)] focus:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                  <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${link.tone}`}><link.icon className="h-5 w-5" /></div>
                  <div className="mt-4 flex items-end justify-between gap-3"><div><p className="font-serif text-xl">{link.label}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{link.description}</p></div><ArrowRight className="h-4 w-4 shrink-0 text-muted-foreground transition-transform group-hover:translate-x-1" /></div>
                </button>
              </DashboardAnimatedCard>
            ))}
          </div>
        ...
        <DashboardAnimatedCard>
          <Card className="nature-card"><CardHeader><div className="flex items-center justify-between gap-4"><div><CardTitle className="flex items-center gap-2 font-serif text-2xl"><Goal className="h-5 w-5 text-primary" /> Upcoming goals</CardTitle><CardDescription>Keep the next step close enough to touch.</CardDescription></div><Button variant="ghost" onClick={() => setLocation("/goals")} className="rounded-full">View all</Button></div></CardHeader><CardContent>{overview?.activeGoals?.length ? <div className="grid gap-3 md:grid-cols-2">{overview.activeGoals.slice(0, 4).map(goal => { const progress = goal.stepCount ? Math.round((goal.completedStepCount / goal.stepCount) * 100) : 0; return <div key={goal.id} className="rounded-2xl border border-border/55 bg-muted/20 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{goal.title}</p><p className="mt-1 text-xs text-muted-foreground">A {goal.horizon}-day horizon</p></div><span className="flex h-7 w-7 items-center justify-center rounded-full bg-primary/10 text-primary"><Check className="h-4 w-4" /></span></div><p className="mt-4 text-xs text-muted-foreground">{goal.nextStepTitle ? `Next: ${goal.nextStepTitle}` : goal.stepCount ? "All steps complete" : "Add a first step when you are ready"}</p>{goal.stepCount ? <><Progress value={progress} className="mt-3 h-1.5" /><p className="mt-2 text-[11px] text-muted-foreground">{goal.completedStepCount} of {goal.stepCount} steps complete</p></> : null}</div>; })}</div> : <div className="rounded-2xl border border-dashed border-border/70 bg-muted/15 p-6 text-center"><p className="font-serif text-xl">No goals waiting yet.</p><p className="mt-1 text-sm text-muted-foreground">When you are ready, choose one small direction for the next stretch.</p><Button onClick={() => setLocation("/goals")} className="mt-4 rounded-full">Create a goal <ArrowRight className="ml-2 h-4 w-4" /></Button></div>}</CardContent></Card>
        </DashboardAnimatedCard>
