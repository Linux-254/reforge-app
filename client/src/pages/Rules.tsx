...
type ReviewState = "due" | "overdue" | "upcoming" | "paused";
...
export default function Rules() {
  ...
  const listQuery = trpc.rules.all.useQuery(undefined, { enabled: !demoMode && Boolean(user) });
  const reviewsQuery = trpc.rules.reviews.useQuery(
    { ruleId: reviewRuleId ?? 0 },
    { enabled: !demoMode && Boolean(user) && reviewRuleId !== null, retry: false },
  );
  ...
  const reviewSummary = useMemo(() => ({
    due: rules.filter(rule => rule.reviewState === "due").length,
    overdue: rules.filter(rule => rule.reviewState === "overdue").length,
      ...
      <div className="mx-auto max-w-6xl space-y-7">
        <section className="relative overflow-hidden rounded-[2rem] border border-border/60 bg-[#304333] text-[#f7eddc]">
          ...
          <div className="relative grid gap-6 p-7 sm:p-10 lg:grid-cols-[1fr_auto] lg:items-end">
            ...
            <div className="rounded-full border border-white/15 bg-black/10 px-4 py-3 text-xs text-white/75"><TimerReset className="mr-2 inline h-4 w-4 text-amber-200" aria-hidden="true" /> Cadence-aware reviews</div>
          </div>
        </section>

        <section aria-label="Boundary review summary" className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-amber-900/10 bg-amber-50/70 p-4 dark:bg-amber-950/20"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Due now</p><p className="mt-1 font-serif text-3xl text-amber-900 dark:text-amber-200">{reviewSummary.due}</p><p className="text-xs text-muted-foreground">New boundaries to review</p></div>
          <div className="rounded-2xl border border-destructive/15 bg-destructive/5 p-4"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Overdue</p><p className="mt-1 font-serif text-3xl text-destructive">{reviewSummary.overdue}</p><p className="text-xs text-muted-foreground">A gentle return point</p></div>
          <div className="rounded-2xl border border-primary/15 bg-primary/5 p-4"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Active</p><p className="mt-1 font-serif text-3xl text-primary">{reviewSummary.active}</p><p className="text-xs text-muted-foreground">Boundaries currently in use</p></div>
        </section>
          ...
          <Card className="rounded-2xl border-border/70 bg-card/80 shadow-none">
            ...
            <CardContent className="space-y-3">
              {listQuery.isError ? <div className="space-y-3"><p className="text-sm text-destructive">We could not load your boundaries right now.</p><Button variant="outline" className="rounded-full" onClick={() => void listQuery.refetch()}>Try again</Button></div> : rules.length ? rules.map((rule) => {
                const state = (rule.reviewState ?? "due") as ReviewState;
                const stateTone = state === "overdue" ? "text-destructive border-destructive/20 bg-destructive/5" : state === "due" ? "text-amber-900 border-amber-900/15 bg-amber-50/70 dark:text-amber-200 dark:bg-amber-950/20" : state === "paused" ? "text-muted-foreground border-border/60 bg-muted/20" : "text-primary border-primary/15 bg-primary/5";
                return <article key={rule.id} className={`rounded-2xl border p-4 transition-colors ${rule.active === false ? "opacity-75" : ""} ${stateTone}`}>
                  {editingId === rule.id ? <div className="space-y-3"><Label htmlFor={`edit-rule-${rule.id}`} className="sr-only">Edit boundary</Label><Input id={`edit-rule-${rule.id}`} value={editingText} onChange={(event) => setEditingText(event.target.value)} maxLength={500} className="rounded-xl bg-background" /><div className="flex flex-wrap gap-2"><select aria-label="Edit review cadence" value={editingCadence} onChange={(event) => setEditingCadence(event.target.value as Cadence)} className="h-9 rounded-full border bg-background px-3 text-xs"><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option></select><Button type="button" size="sm" className="rounded-full" disabled={!editingText.trim() || updateMutation.isPending} onClick={saveEdit}><Save className="mr-1 h-3.5 w-3.5" aria-hidden="true" /> Save</Button><Button type="button" variant="ghost" size="sm" className="rounded-full" onClick={() => setEditingId(null)}><X className="mr-1 h-3.5 w-3.5" aria-hidden="true" /> Cancel</Button></div></div> : <>
                    <div className="flex items-start justify-between gap-4"><div className="flex min-w-0 items-start gap-3"><div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-background/70 text-primary"><Check className="h-3.5 w-3.5" aria-hidden="true" /></div><div className="min-w-0"><p className="text-sm leading-6 text-foreground">{rule.text}</p><div className="mt-2 flex flex-wrap items-center gap-2"><Badge variant="secondary" className="rounded-full text-xs">{CADENCE_LABEL[(rule.reviewCadence as Cadence) ?? "daily"]} review</Badge><Badge variant="outline" className="rounded-full text-xs">{rule.active === false ? "Paused" : "Active"}</Badge></div></div></div><div className="flex shrink-0 items-center gap-1"><Button type="button" variant="ghost" size="icon" className="rounded-full" aria-label={`Edit boundary: ${rule.text}`} onClick={() => beginEdit(rule)}><Pencil className="h-4 w-4" aria-hidden="true" /></Button><Button type="button" variant="ghost" size="icon" className="rounded-full text-destructive hover:text-destructive" aria-label={`Remove boundary: ${rule.text}`} onClick={() => setDeleteId(rule.id)}><Trash2 className="h-4 w-4" aria-hidden="true" /></Button></div></div>
                    <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-border/50 pt-3"><p className="flex items-center gap-1.5 text-xs font-medium"><span className={`inline-flex h-5 w-5 items-center justify-center rounded-full bg-background/80 ${state === "overdue" ? "text-destructive" : state === "due" ? "text-amber-700 dark:text-amber-200" : "text-primary"}`}>{state === "overdue" ? <AlertCircle className="h-3.5 w-3.5" aria-hidden="true" /> : state === "paused" ? <PauseCircle className="h-3.5 w-3.5" aria-hidden="true" /> : <CalendarClock className="h-3.5 w-3.5" aria-hidden="true" />}</span>{REVIEW_STATE_LABEL[state]}{state !== "paused" && <span className="font-normal text-muted-foreground">· {state === "upcoming" ? `next ${formatReviewDate(rule.nextReviewAt)}` : formatReviewDate(rule.nextReviewAt)}</span>}</p><label className="ml-auto flex items-center gap-2 text-xs text-muted-foreground"><input type="checkbox" checked={rule.active !== false} onChange={(event) => updateMutation.mutate({ ruleId: rule.id, active: event.target.checked })} className="h-4 w-4 accent-[hsl(var(--primary))]" aria-label={`${rule.active === false ? "Activate" : "Pause"} boundary: ${rule.text}`} /> Keep active</label><Button type="button" variant="outline" size="sm" className="rounded-full" disabled={rule.active === false} onClick={() => { setReviewRuleId(reviewRuleId === rule.id ? null : rule.id); setReviewNotes(""); }}><ClipboardCheck className="mr-1 h-3.5 w-3.5" aria-hidden="true" /> {reviewRuleId === rule.id ? "Close review" : "Review now"}</Button></div>
                    {reviewRuleId === rule.id && <div className="mt-3 space-y-3 rounded-2xl border border-primary/20 bg-background/60 p-4"><div className="flex items-center gap-2"><History className="h-4 w-4 text-primary" aria-hidden="true" /><p className="text-sm font-medium">How did this boundary hold?</p></div><Textarea value={reviewNotes} onChange={(event) => setReviewNotes(event.target.value)} maxLength={2000} placeholder="Optional private note about what you noticed…" className="min-h-20 rounded-xl bg-background/70" aria-label="Optional private boundary review note" /><div className="flex flex-wrap gap-2"><Button type="button" size="sm" className="rounded-full" disabled={reviewMutation.isPending} onClick={() => recordReview(true)}><CheckCircle2 className="mr-1 h-3.5 w-3.5" aria-hidden="true" /> Kept it</Button><Button type="button" variant="outline" size="sm" className="rounded-full" disabled={reviewMutation.isPending} onClick={() => recordReview(false)}>Needs more support</Button></div>{reviewsQuery.isLoading ? <div className="space-y-2"><Skeleton className="h-12 w-full rounded-xl" /><Skeleton className="h-12 w-full rounded-xl" /></div> : reviewsQuery.isError ? <p className="text-xs text-destructive">Review history is unavailable right now.</p> : reviewsQuery.data?.length ? <div className="space-y-2"><p className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">Recent reviews</p>{reviewsQuery.data.slice(0, 3).map((review) => <div key={review.id} className="rounded-xl bg-muted/35 p-3 text-xs"><div className="flex flex-wrap justify-between gap-2"><span className={review.kept ? "text-primary" : "text-destructive"}>{review.kept ? "Kept" : "Needs support"}</span><span className="text-muted-foreground">{formatReviewDate(review.reviewDate)}</span></div>{review.notes && <p className="mt-1 leading-5 text-muted-foreground">{review.notes}</p>}</div>)}</div> : <p className="text-xs text-muted-foreground">No reviews recorded yet.</p>}</div>}
                  </>}
                </article>;
